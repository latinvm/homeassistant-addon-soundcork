...
if [[ ! -f "${OPTIONS_FILE}" ]]; then
  echo "[soundcork-basicauth] ${OPTIONS_FILE} missing; supervisor did not provide options" >&2
  exit 1
fi
...
if [[ -z "${base_url}" ]]; then
  echo "[soundcork-basicauth] base_url is required (set it on the Configuration tab)" >&2
  exit 1
fi
...
case "${data_dir}" in
  /data | /data/* ) ;;
  * )
    echo "[soundcork-basicauth] data_dir must be under /data (got '${data_dir}')" >&2
    exit 1
    ;;
esac
...
if (( ${#seed_files[@]} == 0 )); then
  echo "[soundcork-basicauth] no seed files found, skipping"
else
  echo "[soundcork-basicauth] seeding ${#seed_files[@]} file(s) from ${SEED_DIR} into /data (existing files preserved)"
  for f in "${seed_files[@]}"; do
    rel=${f#"${SEED_DIR}/"}
    echo "[soundcork-basicauth] seed: ${rel}"
  done
  cp -rn "${SEED_DIR}/." /data/
fi
